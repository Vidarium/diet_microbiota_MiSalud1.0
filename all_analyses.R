...
# Now, let's see whether gut microbiota associates with nutrients separately
glm.nb_all_nutrients = function(x)
  glm.nb(x~znutri$Calorias+znutri$ProtTotal+
           znutri$GT+znutri$GS+znutri$GM+znutri$GP+
           znutri$COLESTEROL+znutri$CHOtotal+znutri$CHOcom+
           znutri$FD+znutri$Ca+znutri$P+znutri$FeTotal+
           znutri$Na+znutri$K+znutri$Mg+znutri$Zn+znutri$Cu+
           znutri$Mn+znutri$VitA.ER.+znutri$B1+znutri$B2+
           znutri$B3+znutri$AcPantoenico+znutri$B6+znutri$Folico+
           znutri$B12+znutri$AcAscorbico+
           metadata$ciudad+metadata$sexo+metadata$rango_edad+
           IMCcat)

glm.nb_all_nutrients = function(x,y)
  glm.nb(x~y+znutri$Calorias+metadata$ciudad+
           metadata$sexo+metadata$rango_edad+IMCcat)

microb_all_nutrients = map(.x = abundant_otus_rare,
                           .y = znutri$ProtTotal,
                           .f = possibly(.f = glm.nb_all_nutrients, otherwise = NA_real_))

microb_all_nutrients_anova = map(.x = microb_all_nutrients,
                                 .f = possibly(.f = function(x) Anova(x), otherwise = NA_real_))

glm_nutr_microb_p = map(.x = glm_nutr_microb_anova,
                        .f = possibly(.f = function(x) cbind(x$`Pr(>Chisq)`), otherwise = NA_real_))

p_nutr_microb = map(.x = glm_nutr_microb_p,
                    .f = possibly(.f = function(x) cbind(x[c(1:28),1]), otherwise = NA_real_))

p_nutr_microb = as.data.frame(p_nutr[c(1:137)])
write.table(p_nutr_microb, file="d:/Vidarium/Publicaciones/Dieta_ASGV/scripts/JSE/xxx.txt", sep="\t", quote=FALSE)



# Now, let's see whether gut microbiota associates with PCA axes
glm_negbin_pca = function(x)
  glm.nb(x~pca_table$pca1+pca_table$pca2+pca_table$pca3+
           metadata$ciudad+metadata$sexo+
           metadata$rango_edad+IMCcat)

glm_pca_microb = map(.x = abundant_otus_rare,
                     .f = possibly(.f = glm_negbin_pca, otherwise = NA_real_))

glm_pca_microb_anova = map(.x = glm_pca_microb,
                           .f = possibly(.f = function(x) Anova(x), otherwise = NA_real_))

glm_pca_microb_p = map(.x = glm_pca_microb_anova,
                       .f = possibly(.f = function(x) cbind(x$`Pr(>Chisq)`), otherwise = NA_real_))

p_pca_microb = map(.x = glm_pca_microb_p,
                   .f = possibly(.f = function(x) cbind(x[c(1:3),1]), otherwise = NA_real_))

p_pca_microb = as.data.frame(p_pca_microb[c(1:137)])
write.table(p_pca_microb, file="d:/Vidarium/Publicaciones/Dieta_ASGV/scripts/JSE/xxx2.txt", sep="\t", quote=FALSE)

## Summary of results: neither individual nutrients, nor PCA axes
# associated with the abundance of gut microbes

# Now, what about alpha diversity?
alphadiv_nutr = function(x) lm(x~znutri$Calorias+znutri$ProtTotal+
                                 znutri$GT+znutri$GS+znutri$GM+znutri$GP+
                                 znutri$COLESTEROL+znutri$CHOtotal+znutri$CHOcom+
                                 znutri$FD+znutri$Ca+znutri$P+znutri$FeTotal+
                                 znutri$Na+znutri$K+znutri$Mg+znutri$Zn+znutri$Cu+
...
lm_alphadiv_nutr = map(.x = metadata[c(161:163)],
                       .f = possibly(.f = alphadiv_nutr, otherwise = NA_real_))

lm_alphadiv_nutr_anova = map(.x = lm_alphadiv_nutr,
                             .f = possibly(.f = function(x) Anova(x), otherwise = NA_real_))

lm_alphadiv_nutr_p = map(.x = lm_alphadiv_nutr_anova,
                         .f = possibly(.f = function(x) cbind(x$`Pr(>F)`), otherwise = NA_real_))

p_alphadiv_nutr = map(.x = lm_alphadiv_nutr_p,
                      .f = possibly(.f = function(x) cbind(x[c(1:28),1]), otherwise = NA_real_))

p_alphadiv_nutr = as.data.frame(p_alphadiv_nutr[c(1:3)])
write.table(p_nutr_microb, file="d:/Vidarium/Publicaciones/Dieta_ASGV/scripts/JSE/xxx.txt", sep="\t", quote=FALSE)

# Results: Cu and ProtTot were significantly associated with alpha diversity.
# Now, let's see what is the direction of the association
lm_alphadiv_nutr_summary = map(.x = lm_alphadiv_nutr,
                                  .f = possibly(.f = function(x) summary(x), otherwise = NA_real_))

lm_alphadiv_nutr_z = map(.x = lm_alphadiv_nutr_summary,
                         .f = possibly(.f = function(x) cbind(x$coefficients[c(3,19),1]), otherwise = NA_real_))

# Total protein intake
ggplot(metadata,aes(x=Shannon,y=znutri$ProtTotal)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(x="Shannon diversity index",y="Standardized total protein intake")

ggplot(metadata,aes(x=Jevenness,y=znutri$ProtTotal)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(x="Pielou's evenness",y="Standardized total protein intake")

# Cupper intake
ggplot(metadata,aes(x=richness,y=znutri$Cu)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(x="OTU richness",y="Standardized Cu intake")

ggplot(metadata,aes(x=Shannon,y=znutri$Cu)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(x="Shannon diversity index",y="Standardized Cu intake")

ggplot(metadata,aes(x=Jevenness,y=znutri$Cu)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(x="Pielou's evenness",y="Standardized Cu intake")

# Conclusion: total protein and cupper intake were positively associated with gut microbiota alpha diversity


# Now, what about associations with CAG abundance?
# associated with the abundance of gut microbes
CAGn5_nutr = function(x) lm(asin(sqrt(x))~znutri$Calorias+znutri$ProtTotal+
                                 znutri$GT+znutri$GS+znutri$GM+znutri$GP+
                                 znutri$COLESTEROL+znutri$CHOtotal+znutri$CHOcom+
                                 znutri$FD+znutri$Ca+znutri$P+znutri$FeTotal+
                                 znutri$Na+znutri$K+znutri$Mg+znutri$Zn+znutri$Cu+
...
lm_CAGn5_nutr = map(.x = CAG_410,
                    .f = possibly(.f = CAGn5_nutr, otherwise = NA_real_))

lm_CAGn5_nutr_anova = map(.x = lm_CAGn5_nutr,
                          .f = possibly(.f = function(x) Anova(x), otherwise = NA_real_))

lm_CAGn5_nutr_p = map(.x = lm_CAGn5_nutr_anova,
                      .f = possibly(.f = function(x) cbind(x$`Pr(>F)`), otherwise = NA_real_))

p_CAGn5_nutr = map(.x = lm_CAGn5_nutr_p,
                   .f = possibly(.f = function(x) cbind(x[c(1:28),1]), otherwise = NA_real_))

p_CAGn5_nutr = as.data.frame(p_CAGn5_nutr[c(1:5)])
write.table(p_nutr_microb, file="d:/Vidarium/Publicaciones/Dieta_ASGV/scripts/JSE/xxx.txt", sep="\t", quote=FALSE)

CAG_n5_matrix

# Random-forest regressions (Boruta) ----
# Create a dataframe of predictors
znutri2<-cbind(metadata[,c(1,2,4)],IMCcat,znutri)

# Define the Boruta function and run it
rf.microb_nutr = function(x)
  Boruta(znutri2, x, doTrace=2)

rf.otus_nutr = map(.x = abundant_otus_rare,
                   .f = possibly(.f = rf.microb_nutr, otherwise = NA_real_))

# Check the selected features per OTU
selattr = function(x) 
  getSelectedAttributes(x, withTentative = F)
rf.otus_nutr_df <- function(x)
  print(attStats(x))

selattr.otus = map(.x = rf.otus_nutr,
                   .f = possibly(.f = selattr, otherwise = NA_real_))

rf.otus_nutr = map(.x = rf.otus_nutr,
                   .f = possibly(.f = rf.otus_nutr_df, otherwise = NA_real_))

sapply(rf.otus_nutr, FUN=function(x) x$finalDecision)


# To do:
# 1. Analyze first 5 PCs vs. microbiota
# 1.1. OTU abundance
# 1.2. CAG abundance
# 1.3. Alpha diversity
...
# 2. Analyze food groups vs. microbiota
# 2.1. OTU abundance
# 2.2. CAG abundance
# 2.3. Alpha diversity
# 2.4. Microbial functions
# 2.5. CAZy enzimes

# 3. Analyze food quality vs. microbiota
# 3.1. OTU abundance
# 3.2. CAG abundance
# 3.3. Alpha diversity
# 3.4. Microbial functions
# 2.5. CAZy enzimes